...
import { Bot, User, Loader2, FileText, X, Book, AlertTriangle, Check } from 'lucide-react';
                 ...
                 <div className="text-sm truncate">
                   ...
                   {contract.resourceLinks && contract.resourceLinks.length > 0 && documentStatus[contract.noticeId] && (
                     <div className="text-xs mt-1">
                       {(() => {
                         const status = documentStatus[contract.noticeId];
                         switch (status.status) {
                           case 'processing':
                             return (
                              <span className="text-blue-600 flex items-center gap-1">
                              <Loader2 className="h-3 w-3 animate-spin" />
                              Processing documents {status.processedCount ? 
                                `(${status.processedCount} of ${status.documentCount})` : 
                                '...'}
                            </span>
                             );
                           case 'completed':
                             return (
                               <span className="text-green-600 flex items-center gap-1">
                                 ...
                                 Documents processed ({status.documentCount})
                               </span>
                             );
                           case 'error':
                             return (
